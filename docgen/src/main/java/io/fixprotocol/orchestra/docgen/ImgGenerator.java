...
public class ImgGenerator {
	...
	public void generateUMLStateMachine(File outputDir, StateMachineType stateMachine, STErrorListener errorListener)
			throws IOException {
	...
	public void generateUMLSequence(File outputDir, MessageType message, FlowType flow, List<ResponseType> responseList,
		...
		SourceStringReader reader = new SourceStringReader(umlString);
		File outputFile = new File(outputDir, String.format("%s-%s.png", message.getName(), message.getScenario()));
		FileOutputStream png = new FileOutputStream(outputFile);
		String desc = reader.generateImage(png);
	}
